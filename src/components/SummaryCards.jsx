...
export default function SummaryCards(){
  ...
  // compute net change over last 7 days vs previous 7 days
  const lastN = (n=7, offset=0)=>{
    const days = []
  ...
  const sumForDays = (days)=> tx.filter(t=> days.includes(t.date)).reduce((s,t)=>s+t.amount,0)
  const thisWeekSum = sumForDays(lastN(7,0))
  const prevWeekSum = sumForDays(lastN(7,7))
  const netChange = thisWeekSum - prevWeekSum
  ...
  const Card = ({label,amount,spark})=>{
    return (
      <div className="card" style={{display:'flex',flexDirection:'column',gap:8,padding:18}}>
        ...
        {spark && (
          <div className="sparkline" style={{opacity:0.95}}>
  ...
  return (
    <div className="summary kpi-grid">
      ...
      <Card label="Net Change (7d)" amount={netChange} spark={sparkData} />
    </div>
  )
}
